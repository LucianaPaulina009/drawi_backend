...
from app.modules.diagramas.application.queries.dtos import ReferenciaFKDTO
...
class ObtenerReferenciaFKQuery:
    propietario_id: str
    relacion_id: UUID
    referencia_id: UUID


class ObtenerReferenciaFKQueryHandler:
    def __init__(
        self,
    ...
    def execute(self, query: ObtenerReferenciaFKQuery) -> ReferenciaFKDTO:
        ...
        referencia = self.referencia_fk_repository.obtener_por_id(query.referencia_id)
        if referencia is None or referencia.id_relacion != query.relacion_id:
            raise ReferenciaFKNoEncontradaException()

        return ReferenciaFKDTO(
            id=referencia.id,
